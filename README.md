...
| `configuration.nix` | System level: bootloader, locale/timezone, greetd + ReGreet login (solid `#292929` via `extraCss`; no background image — static images freeze ReGreet 0.5.0 via GStreamer fallback, nixpkgs#557002), GNOME/Niri sessions, NVIDIA open kernel modules (required for Blackwell RTX 5090 — closed module leaves no `renderD128` and Niri starts output-less), audio (PipeWire), fonts (Fira + Noto incl. Kannada), IBus Wayland frontend (US + Kannada itrans phonetic via `toggle-keyboard-layout`; unset GTK/QT IM vars so Zed/Chrome use text-input-v3), `NIXOS_OZONE_WL=1` for Wayland Chrome/Electron IME, OpenSSH, Tailscale, user account, system packages. |
...
| `niri` | Niri scrollable-tiling session: `config.kdl`, Super-key binds, Noctalia IPC for launcher/volume/media, random wallpaper from `~/Pictures/Wallpapers` on session start, screenshots via grim+slurp+swappy, GTK/Adwaita theming, F11 US/Kannada toggle (Mod+Shift+Space fallback) with Noctalia toast. Single ibus-daemon via XDG autostart/D-Bus — no manual spawn (a second daemon breaks Wayland text-input). | Active desktop; binds reuse `my.terminal`/`my.browser` so switching apps touches one file. |
| `hyprland` | Alternative Hyprland setup (Lua-config syntax): monitors, master layout, animations, same Noctalia-integrated keymap as niri (incl. F11 US/Kannada toggle with toast). IBus daemon via XDG autostart/D-Bus, not manual spawn (see niri row). | Drop-in desktop alternative; kept disabled but ready. |
| `gnome` | GNOME via pop-shell tiling + extensions (caffeine, vitals, burn-my-windows, user-themes) and dconf keybinds mirroring the Super-key scheme. Input sources: us + Kannada itrans (Super+Space, F11). | Third desktop option with the same muscle memory. |
